...
export default function Home() {
  ...
  const handleBodyPartClick = (bodyPart, tool) => {
    if (!tool) {
      // If no tool selected, just open the panel
      setIsPanelOpen(true)
      return
    }
  ...
  const performExamination = (bodyPart, tool) => {
    ...
    return examinations[tool]?.[bodyPart] || `No significant findings in ${bodyPart} with ${tool}`
  }
  ...
  return (
    <main className="h-screen w-full relative">
      ...
      {!isPanelOpen && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-blue-600/80 text-white px-6 py-3 rounded-lg backdrop-blur-sm">
      ...
      <Canvas
        ...
        onClick={() => {
          // Open panel when clicking anywhere on the canvas (patient)
          if (!isPanelOpen) {
            setIsPanelOpen(true)
          }
        }}
